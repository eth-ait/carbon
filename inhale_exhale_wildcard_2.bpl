...
tyoe Perm = Pair real bool;
type MaskType = <A, B> [Ref, Field A B]Perm;
var Mask: MaskType;
const ZeroMask: MaskType;
axiom (forall <A, B> o_1: Ref, f_3: (Field A B) ::
  { ZeroMask[o_1, f_3] }
  fst(ZeroMask[o_1, f_3]) == NoPerm && !snd(ZeroMask)
);
...
function Perm(a: real, b: real, b: bool): Perm;
...
axiom (forall <A, B> Mask: MaskType, o_1: Ref, f_3: (Field A B) ::
  ...
  GoodMask(Mask) ==> fst(Mask[o_1, f_3]) >= NoPerm && ((fst(GoodMask(Mask)) && !IsPredicateField(f_3)) && !IsWandField(f_3) ==> fst(Mask[o_1, f_3]) <= FullPerm)
);
...
axiom (forall <A, B> Mask: MaskType, o_1: Ref, f_3: (Field A B) ::
  { HasDirectPerm(Mask, o_1, f_3) }
  HasDirectPerm(Mask, o_1, f_3) <==> fst(Mask[o_1, f_3]) > NoPerm || (fst(Mask[o_1, f_3]) == NoPerm && snd(Mask[o_1, f_3]))
);
